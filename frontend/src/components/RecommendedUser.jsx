...
const RecommendedUser = ({ user }) => {
  ...
  const {mutate:acceptRequest}=useMutation({
    ...
    onSuccess:()=>{
      toast.success("Conection request accepted"),
      queryClient.invalidateQueries({ queryKey: ["connectionStatus", user._id] })
    },
    onError:(error)=>{
  ...
  const {mutate:rejectRequest}=useMutation({
    ...
    onSuccess:()=>{
      toast.success("Conection request rejected"),
      queryClient.invalidateQueries({ queryKey: ["connectionStatus", user._id] })
    },
    onError:(error)=>{
      toast.error(error.response?.data?.error || "something wrong")
  ...
  const renderButton =()=>{
    ...
    switch (connectionStatus?.data?.status){
      case "pending":
        return(
          <button className='px-3 py-1 rounded-full text-sm bg-yellow-500 text-white flex items-center
          ' disabled>
        ...
        case "connected":
          ...
          default:
            return(
              <button className='px-3 py-1 rounded-full text-sm border border-primary text-primary 
              hover:bg-primary hover:text-white transition-colors duration-200 flex items-center'
